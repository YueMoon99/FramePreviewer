...
LANG = {
    'cn': {
        'title': '序列帧预览器 v2.1 By 舟午YueMoon',
        'fps': '帧速率:',
        'bg': '背景:',
        'zoom': '缩放:',
        'fit': '适应窗口',
    ...
    'en': {
        'title': 'Image Sequence Previewer v2.1 By 舟午YueMoon',
        'fps': 'FPS:',
        'bg': 'Background:',
        'zoom': 'Zoom:',
        'fit': 'Fit Window',
...
class TimelineBar(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # 绘制背景条
        painter.fillRect(0, bar_y, width, bar_height, QColor(60, 60, 60))
        # 绘制缓存条
        cache_width = int((self.cached_frames / self.total_frames) * width)
        painter.fillRect(0, bar_y, cache_width, bar_height, QColor(0, 200, 0))
        ...
        triangle = QPolygonF([
            QPointF(playhead_x - 5, bar_y),
            QPointF(playhead_x + 5, bar_y),
            QPointF(playhead_x, bar_y + 6)
        ])
        ...
        if self.current_label:
            ...
            text_width = fm.horizontalAdvance(self.current_label)
            rect_w = text_width + 12
            rect_h = 14
            
            # 限制气泡不要超出左右边界
            rect_x = max(0, min(playhead_x - rect_w / 2, width - rect_w))
            rect_y = 0
            
            painter.setBrush(QBrush(QColor(0, 0, 0, 180)))
            painter.setPen(Qt.PenStyle.NoPen)
            painter.drawRoundedRect(QRectF(rect_x, rect_y, rect_w, rect_h), 4, 4)
            
            painter.setPen(QColor(255, 255, 255))
            painter.drawText(QRectF(rect_x, rect_y, rect_w, rect_h), Qt.AlignmentFlag.AlignCenter, self.current_label)
